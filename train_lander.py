...
wandb.init(project="ppo-atari-LunarLanderContinuous-v2", name='enropy=0.001_clipNorm=1')
env = gym.vector.make('LunarLanderContinuous-v2', ENV_SIZE).unwrapped
ppo = PPO(policy_args)
...
for current_episode in range(EPISODE):
    ...
    while True:
        ...
        state = next_state
        total_game_step += 1
        # wandb.log({'average_reward_test': average_reward})
